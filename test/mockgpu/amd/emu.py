...
def _compile_mfma(inst: irc.VOP3P, ctx: _Ctx) -> UOp:
  ...
  assert M == N, f"Only square MFMA supported, got {M}x{N}"
  cvt = _FUNCS['bf16_to_f32'] if is_bf16 else _FUNCS['f16_to_f32']
  vpg = 4 if is_fp8 else 2

  total_outputs = M * N
  wave_size = 64
  active_lanes = total_outputs if total_outputs < wave_size else wave_size
  out_per_lane = total_outputs // active_lanes
  groups = active_lanes // M
  k_per_grp = K // groups
  ...
  # Phase 1: Read inputs from VGPRs to local memory
  read_lane = ctx.range(active_lanes)
  ...
  for kl in range(k_per_grp):
    ...
    if is_fp8: a_f = ((a_raw >> UOp.const(dtypes.uint32, sub_idx * 8)) & UOp.const(dtypes.uint32, 0xFF)).cast(dtypes.uint32)
    else: a_f = cvt((a_raw >> UOp.const(dtypes.uint32, sub_idx * 16)) & UOp.const(dtypes.uint32, 0xFFFF))
    ...
    if is_fp8: b_f = ((b_raw >> UOp.const(dtypes.uint32, sub_idx * 8)) & UOp.const(dtypes.uint32, 0xFF)).cast(dtypes.uint32)
    else: b_f = cvt((b_raw >> UOp.const(dtypes.uint32, sub_idx * 16)) & UOp.const(dtypes.uint32, 0xFFFF))
  ...
  compute_lane = ctx.range(active_lanes)
  n_idx = compute_lane % UOp.const(dtypes.int, M)
  c_grp = compute_lane // UOp.const(dtypes.int, M)
...
def _compile_vop3p(inst: ir3.VOP3P | ir4.VOP3P | irc.VOP3P, ctx: _Ctx) -> UOp:
  ...
  if isinstance(inst, irc.VOP3P_MFMA): return _compile_mfma(inst, ctx)
